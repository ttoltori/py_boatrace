...
class BoatCatboostRankerTest:
    ...
    def execute(self, param_list_str, csv_filepath, model_filepath, feature_name_list_str, feature_type_list_str):
        ...
        train_pool = Pool(data=X_train, label=y_train, group_id=X_train['raceid'], cat_features=cat_features)
        val_pool = Pool(data=X_val, label=y_val, group_id=X_val['raceid'], cat_features=cat_features)
